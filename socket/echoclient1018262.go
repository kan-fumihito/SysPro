...
func tcp_connect(server string, portno string) (*net.TCPConn, error) {
	network := "tcp"
	addr, err := net.ResolveTCPAddr(network, server+":"+portno)
	if err != nil {
		return nil, err
	}
...
func echo_client_one(server string, portno string, message string) {
